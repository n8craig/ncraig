...
When launching out to try to understand how to access API's via R I quickly encountered pages that discussed `httr`. It seems to be a very common way of accessing API's via R, and thus worth some mention. The library's "Getting started with `httr`" [page](https://httr.r-lib.org/articles/quickstart.html) is the best place to start understanding how to use it. An R-bloggers [post](https://www.r-bloggers.com/2015/02/using-the-httr-package-to-retrieve-data-from-apis-in-r/) on "using the httr package to retrieve data from apis in R" is also very helpful.

I started out using `httr` before reasoning that I could simply use `read_csv`. There are some working `httr` examples below.

## Request to GDELT using `httr`: table embedded in JSON

Using the `GET()` method it was possible to issue a request to GEDELT 2.0 TV and receive JSON data. If `format` is set to `csv` it will return CSV that just goes back to `read_csv`. In this case, it is easier to just pass the request URL directly to `read_csv`.
...
If the request URL is set to `format=json` the data are returned but in nested json. The data of interest are broken into components and buried in a nested list. Assembling the desired table from this result would probably require `lapply` or `sapply` and `bind_rows`. The entities can be inspected with `pluck`. The first item in the list is a list of the station names. The second item in the list is another list consisting of individual data frames for each of the stations. It would be good in principle to know how to reassemble the desired table from the JSON data, but in this context it seems unnecessary because there are simpler methods to obtain the data table.
...
It is possible to build a request URL based on a set of input parameters. That request can then be passed to `read_csv` or `GET`.

GDELT has a nice interface that lets users know what data set they searched and the details of the search term. The summary URL is a really nice general reference to the search from which a request URL can be created. However, some API's may not have a similar interface.
...
That seems like a lot of work to get the data when the `request` variable can just be passed to `read_csv` and return a data frame.
